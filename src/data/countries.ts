import type { CountryMeta } from '../types/economics'

/**
 * List of major global economies tracked by World Bank and central bank FX providers.
 * Covers over 95% of world nominal GDP.
 */
export const COUNTRIES: CountryMeta[] = [
  {
    id: 'USA',
    iso2: 'US',
    nameKo: '미국',
    nameEn: 'United States',
    currencyCode: 'USD',
    currencyNameEn: 'US Dollar',
    currencyNameKo: '미국 달러',
    currencySymbol: '$',
    region: 'Americas',
    flagEmoji: '🇺🇸',
  },
  {
    id: 'CHN',
    iso2: 'CN',
    nameKo: '중국',
    nameEn: 'China',
    currencyCode: 'CNY',
    currencyNameEn: 'Chinese Yuan',
    currencyNameKo: '중국 위안',
    currencySymbol: '¥',
    region: 'Asia',
    flagEmoji: '🇨🇳',
  },
  {
    id: 'DEU',
    iso2: 'DE',
    nameKo: '독일',
    nameEn: 'Germany',
    currencyCode: 'EUR',
    currencyNameEn: 'Euro',
    currencyNameKo: '유로',
    currencySymbol: '€',
    region: 'Europe',
    flagEmoji: '🇩🇪',
  },
  {
    id: 'JPN',
    iso2: 'JP',
    nameKo: '일본',
    nameEn: 'Japan',
    currencyCode: 'JPY',
    currencyNameEn: 'Japanese Yen',
    currencyNameKo: '일본 엔',
    currencySymbol: '¥',
    region: 'Asia',
    flagEmoji: '🇯🇵',
  },
  {
    id: 'IND',
    iso2: 'IN',
    nameKo: '인도',
    nameEn: 'India',
    currencyCode: 'INR',
    currencyNameEn: 'Indian Rupee',
    currencyNameKo: '인도 루피',
    currencySymbol: '₹',
    region: 'Asia',
    flagEmoji: '🇮🇳',
  },
  {
    id: 'GBR',
    iso2: 'GB',
    nameKo: '영국',
    nameEn: 'United Kingdom',
    currencyCode: 'GBP',
    currencyNameEn: 'British Pound',
    currencyNameKo: '영국 파운드',
    currencySymbol: '£',
    region: 'Europe',
    flagEmoji: '🇬🇧',
  },
  {
    id: 'FRA',
    iso2: 'FR',
    nameKo: '프랑스',
    nameEn: 'France',
    currencyCode: 'EUR',
    currencyNameEn: 'Euro',
    currencyNameKo: '유로',
    currencySymbol: '€',
    region: 'Europe',
    flagEmoji: '🇫🇷',
  },
  {
    id: 'ITA',
    iso2: 'IT',
    nameKo: '이탈리아',
    nameEn: 'Italy',
    currencyCode: 'EUR',
    currencyNameEn: 'Euro',
    currencyNameKo: '유로',
    currencySymbol: '€',
    region: 'Europe',
    flagEmoji: '🇮🇹',
  },
  {
    id: 'BRA',
    iso2: 'BR',
    nameKo: '브라질',
    nameEn: 'Brazil',
    currencyCode: 'BRL',
    currencyNameEn: 'Brazilian Real',
    currencyNameKo: '브라질 헤알',
    currencySymbol: 'R$',
    region: 'Americas',
    flagEmoji: '🇧🇷',
  },
  {
    id: 'CAN',
    iso2: 'CA',
    nameKo: '캐나다',
    nameEn: 'Canada',
    currencyCode: 'CAD',
    currencyNameEn: 'Canadian Dollar',
    currencyNameKo: '캐나다 달러',
    currencySymbol: 'CA$',
    region: 'Americas',
    flagEmoji: '🇨🇦',
  },
  {
    id: 'RUS',
    iso2: 'RU',
    nameKo: '러시아',
    nameEn: 'Russia',
    currencyCode: 'RUB',
    currencyNameEn: 'Russian Ruble',
    currencyNameKo: '러시아 루블',
    currencySymbol: '₽',
    region: 'Europe',
    flagEmoji: '🇷🇺',
  },
  {
    id: 'MEX',
    iso2: 'MX',
    nameKo: '멕시코',
    nameEn: 'Mexico',
    currencyCode: 'MXN',
    currencyNameEn: 'Mexican Peso',
    currencyNameKo: '멕시코 페소',
    currencySymbol: 'Mex$',
    region: 'Americas',
    flagEmoji: '🇲🇽',
  },
  {
    id: 'AUS',
    iso2: 'AU',
    nameKo: '호주',
    nameEn: 'Australia',
    currencyCode: 'AUD',
    currencyNameEn: 'Australian Dollar',
    currencyNameKo: '호주 달러',
    currencySymbol: 'A$',
    region: 'Oceania',
    flagEmoji: '🇦🇺',
  },
  {
    id: 'KOR',
    iso2: 'KR',
    nameKo: '대한민국',
    nameEn: 'South Korea',
    currencyCode: 'KRW',
    currencyNameEn: 'South Korean Won',
    currencyNameKo: '대한민국 원',
    currencySymbol: '₩',
    region: 'Asia',
    flagEmoji: '🇰🇷',
  },
  {
    id: 'ESP',
    iso2: 'ES',
    nameKo: '스페인',
    nameEn: 'Spain',
    currencyCode: 'EUR',
    currencyNameEn: 'Euro',
    currencyNameKo: '유로',
    currencySymbol: '€',
    region: 'Europe',
    flagEmoji: '🇪🇸',
  },
  {
    id: 'IDN',
    iso2: 'ID',
    nameKo: '인도네시아',
    nameEn: 'Indonesia',
    currencyCode: 'IDR',
    currencyNameEn: 'Indonesian Rupiah',
    currencyNameKo: '인도네시아 루피아',
    currencySymbol: 'Rp',
    region: 'Asia',
    flagEmoji: '🇮🇩',
  },
  {
    id: 'TUR',
    iso2: 'TR',
    nameKo: '튀르키예',
    nameEn: 'Turkey',
    currencyCode: 'TRY',
    currencyNameEn: 'Turkish Lira',
    currencyNameKo: '튀르키예 리라',
    currencySymbol: '₺',
    region: 'Europe',
    flagEmoji: '🇹🇷',
  },
  {
    id: 'NLD',
    iso2: 'NL',
    nameKo: '네덜란드',
    nameEn: 'Netherlands',
    currencyCode: 'EUR',
    currencyNameEn: 'Euro',
    currencyNameKo: '유로',
    currencySymbol: '€',
    region: 'Europe',
    flagEmoji: '🇳🇱',
  },
  {
    id: 'SAU',
    iso2: 'SA',
    nameKo: '사우디아라비아',
    nameEn: 'Saudi Arabia',
    currencyCode: 'SAR',
    currencyNameEn: 'Saudi Riyal',
    currencyNameKo: '사우디 리얄',
    currencySymbol: '﷼',
    region: 'Asia',
    flagEmoji: '🇸🇦',
  },
  {
    id: 'CHE',
    iso2: 'CH',
    nameKo: '스위스',
    nameEn: 'Switzerland',
    currencyCode: 'CHF',
    currencyNameEn: 'Swiss Franc',
    currencyNameKo: '스위스 프랑',
    currencySymbol: 'CHF',
    region: 'Europe',
    flagEmoji: '🇨🇭',
  },
  {
    id: 'POL',
    iso2: 'PL',
    nameKo: '폴란드',
    nameEn: 'Poland',
    currencyCode: 'PLN',
    currencyNameEn: 'Polish Zloty',
    currencyNameKo: '폴란드 즈워티',
    currencySymbol: 'zł',
    region: 'Europe',
    flagEmoji: '🇵🇱',
  },
  {
    id: 'SWE',
    iso2: 'SE',
    nameKo: '스웨덴',
    nameEn: 'Sweden',
    currencyCode: 'SEK',
    currencyNameEn: 'Swedish Krona',
    currencyNameKo: '스웨덴 크로나',
    currencySymbol: 'kr',
    region: 'Europe',
    flagEmoji: '🇸🇪',
  },
  {
    id: 'BEL',
    iso2: 'BE',
    nameKo: '벨기에',
    nameEn: 'Belgium',
    currencyCode: 'EUR',
    currencyNameEn: 'Euro',
    currencyNameKo: '유로',
    currencySymbol: '€',
    region: 'Europe',
    flagEmoji: '🇧🇪',
  },
  {
    id: 'ARG',
    iso2: 'AR',
    nameKo: '아르헨티나',
    nameEn: 'Argentina',
    currencyCode: 'ARS',
    currencyNameEn: 'Argentine Peso',
    currencyNameKo: '아르헨티나 페소',
    currencySymbol: '$',
    region: 'Americas',
    flagEmoji: '🇦🇷',
  },
  {
    id: 'IRL',
    iso2: 'IE',
    nameKo: '아일랜드',
    nameEn: 'Ireland',
    currencyCode: 'EUR',
    currencyNameEn: 'Euro',
    currencyNameKo: '유로',
    currencySymbol: '€',
    region: 'Europe',
    flagEmoji: '🇮🇪',
  },
  {
    id: 'NOR',
    iso2: 'NO',
    nameKo: '노르웨이',
    nameEn: 'Norway',
    currencyCode: 'NOK',
    currencyNameEn: 'Norwegian Krone',
    currencyNameKo: '노르웨이 크로네',
    currencySymbol: 'kr',
    region: 'Europe',
    flagEmoji: '🇳🇴',
  },
  {
    id: 'AUT',
    iso2: 'AT',
    nameKo: '오스트리아',
    nameEn: 'Austria',
    currencyCode: 'EUR',
    currencyNameEn: 'Euro',
    currencyNameKo: '유로',
    currencySymbol: '€',
    region: 'Europe',
    flagEmoji: '🇦🇹',
  },
  {
    id: 'ISR',
    iso2: 'IL',
    nameKo: '이스라엘',
    nameEn: 'Israel',
    currencyCode: 'ILS',
    currencyNameEn: 'Israeli New Shekel',
    currencyNameKo: '이스라엘 셰켈',
    currencySymbol: '₪',
    region: 'Asia',
    flagEmoji: '🇮🇱',
  },
  {
    id: 'ARE',
    iso2: 'AE',
    nameKo: '아랍에미리트',
    nameEn: 'United Arab Emirates',
    currencyCode: 'AED',
    currencyNameEn: 'UAE Dirham',
    currencyNameKo: 'UAE 디르함',
    currencySymbol: 'د.إ',
    region: 'Asia',
    flagEmoji: '🇦🇪',
  },
  {
    id: 'SGP',
    iso2: 'SG',
    nameKo: '싱가포르',
    nameEn: 'Singapore',
    currencyCode: 'SGD',
    currencyNameEn: 'Singapore Dollar',
    currencyNameKo: '싱가포르 달러',
    currencySymbol: 'S$',
    region: 'Asia',
    flagEmoji: '🇸🇬',
  },
  {
    id: 'MYS',
    iso2: 'MY',
    nameKo: '말레이시아',
    nameEn: 'Malaysia',
    currencyCode: 'MYR',
    currencyNameEn: 'Malaysian Ringgit',
    currencyNameKo: '말레이시아 링깃',
    currencySymbol: 'RM',
    region: 'Asia',
    flagEmoji: '🇲🇾',
  },
  {
    id: 'ZAF',
    iso2: 'ZA',
    nameKo: '남아프리카 공화국',
    nameEn: 'South Africa',
    currencyCode: 'ZAR',
    currencyNameEn: 'South African Rand',
    currencyNameKo: '남아공 랜드',
    currencySymbol: 'R',
    region: 'Africa',
    flagEmoji: '🇿🇦',
  },
  {
    id: 'PHL',
    iso2: 'PH',
    nameKo: '필리핀',
    nameEn: 'Philippines',
    currencyCode: 'PHP',
    currencyNameEn: 'Philippine Peso',
    currencyNameKo: '필리핀 페소',
    currencySymbol: '₱',
    region: 'Asia',
    flagEmoji: '🇵🇭',
  },
  {
    id: 'DNK',
    iso2: 'DK',
    nameKo: '덴마크',
    nameEn: 'Denmark',
    currencyCode: 'DKK',
    currencyNameEn: 'Danish Krone',
    currencyNameKo: '덴마크 크로네',
    currencySymbol: 'kr',
    region: 'Europe',
    flagEmoji: '🇩🇰',
  },
  {
    id: 'EGY',
    iso2: 'EG',
    nameKo: '이집트',
    nameEn: 'Egypt',
    currencyCode: 'EGP',
    currencyNameEn: 'Egyptian Pound',
    currencyNameKo: '이집트 파운드',
    currencySymbol: 'E£',
    region: 'Africa',
    flagEmoji: '🇪🇬',
  },
  {
    id: 'BGD',
    iso2: 'BD',
    nameKo: '방글라데시',
    nameEn: 'Bangladesh',
    currencyCode: 'BDT',
    currencyNameEn: 'Bangladeshi Taka',
    currencyNameKo: '방글라데시 타카',
    currencySymbol: '৳',
    region: 'Asia',
    flagEmoji: '🇧🇩',
  },
  {
    id: 'VNM',
    iso2: 'VN',
    nameKo: '베트남',
    nameEn: 'Vietnam',
    currencyCode: 'VND',
    currencyNameEn: 'Vietnamese Dong',
    currencyNameKo: '베트남 동',
    currencySymbol: '₫',
    region: 'Asia',
    flagEmoji: '🇻🇳',
  },
  {
    id: 'NGA',
    iso2: 'NG',
    nameKo: '나이지리아',
    nameEn: 'Nigeria',
    currencyCode: 'NGN',
    currencyNameEn: 'Nigerian Naira',
    currencyNameKo: '나이지리아 나이라',
    currencySymbol: '₦',
    region: 'Africa',
    flagEmoji: '🇳🇬',
  },
  {
    id: 'THA',
    iso2: 'TH',
    nameKo: '태국',
    nameEn: 'Thailand',
    currencyCode: 'THB',
    currencyNameEn: 'Thai Baht',
    currencyNameKo: '태국 바트',
    currencySymbol: '฿',
    region: 'Asia',
    flagEmoji: '🇹🇭',
  },
  {
    id: 'HKG',
    iso2: 'HK',
    nameKo: '홍콩',
    nameEn: 'Hong Kong',
    currencyCode: 'HKD',
    currencyNameEn: 'Hong Kong Dollar',
    currencyNameKo: '홍콩 달러',
    currencySymbol: 'HK$',
    region: 'Asia',
    flagEmoji: '🇭🇰',
  },
  {
    id: 'NZL',
    iso2: 'NZ',
    nameKo: '뉴질랜드',
    nameEn: 'New Zealand',
    currencyCode: 'NZD',
    currencyNameEn: 'New Zealand Dollar',
    currencyNameKo: '뉴질랜드 달러',
    currencySymbol: 'NZ$',
    region: 'Oceania',
    flagEmoji: '🇳🇿',
  },
  {
    id: 'FIN',
    iso2: 'FI',
    nameKo: '핀란드',
    nameEn: 'Finland',
    currencyCode: 'EUR',
    currencyNameEn: 'Euro',
    currencyNameKo: '유로',
    currencySymbol: '€',
    region: 'Europe',
    flagEmoji: '🇫🇮',
  },
  {
    id: 'PRT',
    iso2: 'PT',
    nameKo: '포르투갈',
    nameEn: 'Portugal',
    currencyCode: 'EUR',
    currencyNameEn: 'Euro',
    currencyNameKo: '유로',
    currencySymbol: '€',
    region: 'Europe',
    flagEmoji: '🇵🇹',
  },
  {
    id: 'COL',
    iso2: 'CO',
    nameKo: '콜롬비아',
    nameEn: 'Colombia',
    currencyCode: 'COP',
    currencyNameEn: 'Colombian Peso',
    currencyNameKo: '콜롬비아 페소',
    currencySymbol: 'Col$',
    region: 'Americas',
    flagEmoji: '🇨🇴',
  },
  {
    id: 'CHL',
    iso2: 'CL',
    nameKo: '칠레',
    nameEn: 'Chile',
    currencyCode: 'CLP',
    currencyNameEn: 'Chilean Peso',
    currencyNameKo: '칠레 페소',
    currencySymbol: 'CLP$',
    region: 'Americas',
    flagEmoji: '🇨🇱',
  },
  {
    id: 'CZE',
    iso2: 'CZ',
    nameKo: '체코',
    nameEn: 'Czech Republic',
    currencyCode: 'CZK',
    currencyNameEn: 'Czech Koruna',
    currencyNameKo: '체코 코루나',
    currencySymbol: 'Kč',
    region: 'Europe',
    flagEmoji: '🇨🇿',
  },
  {
    id: 'ROU',
    iso2: 'RO',
    nameKo: '루마니아',
    nameEn: 'Romania',
    currencyCode: 'RON',
    currencyNameEn: 'Romanian Leu',
    currencyNameKo: '루마니아 레우',
    currencySymbol: 'lei',
    region: 'Europe',
    flagEmoji: '🇷🇴',
  },
  {
    id: 'PER',
    iso2: 'PE',
    nameKo: '페루',
    nameEn: 'Peru',
    currencyCode: 'PEN',
    currencyNameEn: 'Peruvian Sol',
    currencyNameKo: '페루 솔',
    currencySymbol: 'S/',
    region: 'Americas',
    flagEmoji: '🇵🇪',
  },
  {
    id: 'GRC',
    iso2: 'GR',
    nameKo: '그리스',
    nameEn: 'Greece',
    currencyCode: 'EUR',
    currencyNameEn: 'Euro',
    currencyNameKo: '유로',
    currencySymbol: '€',
    region: 'Europe',
    flagEmoji: '🇬🇷',
  },
  {
    id: 'QAT',
    iso2: 'QA',
    nameKo: '카타르',
    nameEn: 'Qatar',
    currencyCode: 'QAR',
    currencyNameEn: 'Qatari Riyal',
    currencyNameKo: '카타르 리얄',
    currencySymbol: 'QR',
    region: 'Asia',
    flagEmoji: '🇶🇦',
  },
  {
    id: 'HUN',
    iso2: 'HU',
    nameKo: '헝가리',
    nameEn: 'Hungary',
    currencyCode: 'HUF',
    currencyNameEn: 'Hungarian Forint',
    currencyNameKo: '헝가리 포린트',
    currencySymbol: 'Ft',
    region: 'Europe',
    flagEmoji: '🇭🇺',
  },
  {
    id: 'KWT',
    iso2: 'KW',
    nameKo: '쿠웨이트',
    nameEn: 'Kuwait',
    currencyCode: 'KWD',
    currencyNameEn: 'Kuwaiti Dinar',
    currencyNameKo: '쿠웨이트 디나르',
    currencySymbol: 'KD',
    region: 'Asia',
    flagEmoji: '🇰🇼',
  },
  {
    id: 'UKR',
    iso2: 'UA',
    nameKo: '우크라이나',
    nameEn: 'Ukraine',
    currencyCode: 'UAH',
    currencyNameEn: 'Ukrainian Hryvnia',
    currencyNameKo: '우크라이나 흐리우냐',
    currencySymbol: '₴',
    region: 'Europe',
    flagEmoji: '🇺🇦',
  },
  {
    id: 'TWN',
    iso2: 'TW',
    nameKo: '대만',
    nameEn: 'Taiwan',
    currencyCode: 'TWD',
    currencyNameEn: 'New Taiwan Dollar',
    currencyNameKo: '신 대만 달러',
    currencySymbol: 'NT$',
    region: 'Asia',
    flagEmoji: '🇹🇼',
  },
  {
    id: 'PAK',
    iso2: 'PK',
    nameKo: '파키스탄',
    nameEn: 'Pakistan',
    currencyCode: 'PKR',
    currencyNameEn: 'Pakistani Rupee',
    currencyNameKo: '파키스탄 루피',
    currencySymbol: '₨',
    region: 'Asia',
    flagEmoji: '🇵🇰',
  },
  {
    id: 'KAZ',
    iso2: 'KZ',
    nameKo: '카자흐스탄',
    nameEn: 'Kazakhstan',
    currencyCode: 'KZT',
    currencyNameEn: 'Kazakhstani Tenge',
    currencyNameKo: '카자흐스탄 텡게',
    currencySymbol: '₸',
    region: 'Asia',
    flagEmoji: '🇰🇿',
  },
  {
    id: 'IRN',
    iso2: 'IR',
    nameKo: '이란',
    nameEn: 'Iran',
    currencyCode: 'IRR',
    currencyNameEn: 'Iranian Rial',
    currencyNameKo: '이란 리얄',
    currencySymbol: '﷼',
    region: 'Asia',
    flagEmoji: '🇮🇷',
  },
  {
    id: 'IRQ',
    iso2: 'IQ',
    nameKo: '이라크',
    nameEn: 'Iraq',
    currencyCode: 'IQD',
    currencyNameEn: 'Iraqi Dinar',
    currencyNameKo: '이라크 디나르',
    currencySymbol: 'ع.د',
    region: 'Asia',
    flagEmoji: '🇮🇶',
  },
  {
    id: 'DZA',
    iso2: 'DZ',
    nameKo: '알제리',
    nameEn: 'Algeria',
    currencyCode: 'DZD',
    currencyNameEn: 'Algerian Dinar',
    currencyNameKo: '알제리 디나르',
    currencySymbol: 'د.ج',
    region: 'Africa',
    flagEmoji: '🇩🇿',
  },
  {
    id: 'MAR',
    iso2: 'MA',
    nameKo: '모로코',
    nameEn: 'Morocco',
    currencyCode: 'MAD',
    currencyNameEn: 'Moroccan Dirham',
    currencyNameKo: '모로코 디르함',
    currencySymbol: 'د.م.',
    region: 'Africa',
    flagEmoji: '🇲🇦',
  },
  {
    id: 'ETH',
    iso2: 'ET',
    nameKo: '에티오피아',
    nameEn: 'Ethiopia',
    currencyCode: 'ETB',
    currencyNameEn: 'Ethiopian Birr',
    currencyNameKo: '에티오피아 비르',
    currencySymbol: 'Br',
    region: 'Africa',
    flagEmoji: '🇪🇹',
  },
  {
    id: 'SVK',
    iso2: 'SK',
    nameKo: '슬로바키아',
    nameEn: 'Slovakia',
    currencyCode: 'EUR',
    currencyNameEn: 'Euro',
    currencyNameKo: '유로',
    currencySymbol: '€',
    region: 'Europe',
    flagEmoji: '🇸🇰',
  },
  {
    id: 'PRI',
    iso2: 'PR',
    nameKo: '푸에르토리코',
    nameEn: 'Puerto Rico',
    currencyCode: 'USD',
    currencyNameEn: 'US Dollar',
    currencyNameKo: '미국 달러',
    currencySymbol: '$',
    region: 'Americas',
    flagEmoji: '🇵🇷',
  },
  {
    id: 'DOM',
    iso2: 'DO',
    nameKo: '도미니카 공화국',
    nameEn: 'Dominican Republic',
    currencyCode: 'DOP',
    currencyNameEn: 'Dominican Peso',
    currencyNameKo: '도미니카 페소',
    currencySymbol: 'RD$',
    region: 'Americas',
    flagEmoji: '🇩🇴',
  },
  {
    id: 'ECU',
    iso2: 'EC',
    nameKo: '에콰도르',
    nameEn: 'Ecuador',
    currencyCode: 'USD',
    currencyNameEn: 'US Dollar',
    currencyNameKo: '미국 달러',
    currencySymbol: '$',
    region: 'Americas',
    flagEmoji: '🇪🇨',
  },
  {
    id: 'UZB',
    iso2: 'UZ',
    nameKo: '우즈베키스탄',
    nameEn: 'Uzbekistan',
    currencyCode: 'UZS',
    currencyNameEn: 'Uzbekistani Som',
    currencyNameKo: '우즈베키스탄 솜',
    currencySymbol: 'лв',
    region: 'Asia',
    flagEmoji: '🇺🇿',
  },
  {
    id: 'VEN',
    iso2: 'VE',
    nameKo: '베네수엘라',
    nameEn: 'Venezuela',
    currencyCode: 'VES',
    currencyNameEn: 'Venezuelan Bolívar',
    currencyNameKo: '베네수엘라 볼리바르',
    currencySymbol: 'Bs.',
    region: 'Americas',
    flagEmoji: '🇻🇪',
  },
  {
    id: 'AGO',
    iso2: 'AO',
    nameKo: '앙골라',
    nameEn: 'Angola',
    currencyCode: 'AOA',
    currencyNameEn: 'Angolan Kwanza',
    currencyNameKo: '앙골라 콴자',
    currencySymbol: 'Kz',
    region: 'Africa',
    flagEmoji: '🇦🇴',
  },
  {
    id: 'KEN',
    iso2: 'KE',
    nameKo: '케냐',
    nameEn: 'Kenya',
    currencyCode: 'KES',
    currencyNameEn: 'Kenyan Shilling',
    currencyNameKo: '케냐 실링',
    currencySymbol: 'KSh',
    region: 'Africa',
    flagEmoji: '🇰🇪',
  },
  {
    id: 'BGR',
    iso2: 'BG',
    nameKo: '불가리아',
    nameEn: 'Bulgaria',
    currencyCode: 'BGN',
    currencyNameEn: 'Bulgarian Lev',
    currencyNameKo: '불가리아 레프',
    currencySymbol: 'лв',
    region: 'Europe',
    flagEmoji: '🇧🇬',
  },
  {
    id: 'GTM',
    iso2: 'GT',
    nameKo: '과테말라',
    nameEn: 'Guatemala',
    currencyCode: 'GTQ',
    currencyNameEn: 'Guatemalan Quetzal',
    currencyNameKo: '과테말라 케찰',
    currencySymbol: 'Q',
    region: 'Americas',
    flagEmoji: '🇬🇹',
  },
  {
    id: 'LKA',
    iso2: 'LK',
    nameKo: '스리랑카',
    nameEn: 'Sri Lanka',
    currencyCode: 'LKR',
    currencyNameEn: 'Sri Lankan Rupee',
    currencyNameKo: '스리랑카 루피',
    currencySymbol: 'Rs',
    region: 'Asia',
    flagEmoji: '🇱🇰',
  },
  {
    id: 'CRI',
    iso2: 'CR',
    nameKo: '코스타리카',
    nameEn: 'Costa Rica',
    currencyCode: 'CRC',
    currencyNameEn: 'Costa Rican Colón',
    currencyNameKo: '코스타리카 콜론',
    currencySymbol: '₡',
    region: 'Americas',
    flagEmoji: '🇨🇷',
  },
  {
    id: 'LUX',
    iso2: 'LU',
    nameKo: '룩셈부르크',
    nameEn: 'Luxembourg',
    currencyCode: 'EUR',
    currencyNameEn: 'Euro',
    currencyNameKo: '유로',
    currencySymbol: '€',
    region: 'Europe',
    flagEmoji: '🇱🇺',
  },
  {
    id: 'HRV',
    iso2: 'HR',
    nameKo: '크로아티아',
    nameEn: 'Croatia',
    currencyCode: 'EUR',
    currencyNameEn: 'Euro',
    currencyNameKo: '유로',
    currencySymbol: '€',
    region: 'Europe',
    flagEmoji: '🇭🇷',
  },
  {
    id: 'SRB',
    iso2: 'RS',
    nameKo: '세르비아',
    nameEn: 'Serbia',
    currencyCode: 'RSD',
    currencyNameEn: 'Serbian Dinar',
    currencyNameKo: '세르비아 디나르',
    currencySymbol: 'дин.',
    region: 'Europe',
    flagEmoji: '🇷🇸',
  },
  {
    id: 'CIV',
    iso2: 'CI',
    nameKo: '코트디부아르',
    nameEn: "Côte d'Ivoire",
    currencyCode: 'XOF',
    currencyNameEn: 'West African CFA Franc',
    currencyNameKo: '서아프리카 CFA 프랑',
    currencySymbol: 'CFA',
    region: 'Africa',
    flagEmoji: '🇨🇮',
  },
  {
    id: 'PAN',
    iso2: 'PA',
    nameKo: '파나마',
    nameEn: 'Panama',
    currencyCode: 'USD',
    currencyNameEn: 'US Dollar',
    currencyNameKo: '미국 달러',
    currencySymbol: '$',
    region: 'Americas',
    flagEmoji: '🇵🇦',
  },
  {
    id: 'LTU',
    iso2: 'LT',
    nameKo: '리투아니아',
    nameEn: 'Lithuania',
    currencyCode: 'EUR',
    currencyNameEn: 'Euro',
    currencyNameKo: '유로',
    currencySymbol: '€',
    region: 'Europe',
    flagEmoji: '🇱🇹',
  },
  {
    id: 'GHA',
    iso2: 'GH',
    nameKo: '가나',
    nameEn: 'Ghana',
    currencyCode: 'GHS',
    currencyNameEn: 'Ghanaian Cedi',
    currencyNameKo: '가나 세디',
    currencySymbol: 'GH₵',
    region: 'Africa',
    flagEmoji: '🇬🇭',
  },
  {
    id: 'URY',
    iso2: 'UY',
    nameKo: '우루과이',
    nameEn: 'Uruguay',
    currencyCode: 'UYU',
    currencyNameEn: 'Uruguayan Peso',
    currencyNameKo: '우루과이 페소',
    currencySymbol: '$U',
    region: 'Americas',
    flagEmoji: '🇺🇾',
  },
  {
    id: 'TZA',
    iso2: 'TZ',
    nameKo: '탄자니아',
    nameEn: 'Tanzania',
    currencyCode: 'TZS',
    currencyNameEn: 'Tanzanian Shilling',
    currencyNameKo: '탄자니아 실링',
    currencySymbol: 'TSh',
    region: 'Africa',
    flagEmoji: '🇹🇿',
  },
  {
    id: 'BLR',
    iso2: 'BY',
    nameKo: '벨라루스',
    nameEn: 'Belarus',
    currencyCode: 'BYN',
    currencyNameEn: 'Belarusian Ruble',
    currencyNameKo: '벨라루스 루블',
    currencySymbol: 'Br',
    region: 'Europe',
    flagEmoji: '🇧🇾',
  },
  {
    id: 'MMR',
    iso2: 'MM',
    nameKo: '미얀마',
    nameEn: 'Myanmar',
    currencyCode: 'MMK',
    currencyNameEn: 'Myanmar Kyat',
    currencyNameKo: '미얀마 짯',
    currencySymbol: 'K',
    region: 'Asia',
    flagEmoji: '🇲🇲',
  },
  {
    id: 'COD',
    iso2: 'CD',
    nameKo: '콩고민주공화국',
    nameEn: 'DR Congo',
    currencyCode: 'CDF',
    currencyNameEn: 'Congolese Franc',
    currencyNameKo: '콩고 프랑',
    currencySymbol: 'FC',
    region: 'Africa',
    flagEmoji: '🇨🇩',
  },
  {
    id: 'AZE',
    iso2: 'AZ',
    nameKo: '아제르바이잔',
    nameEn: 'Azerbaijan',
    currencyCode: 'AZN',
    currencyNameEn: 'Azerbaijani Manat',
    currencyNameKo: '아제르바이잔 마나트',
    currencySymbol: '₼',
    region: 'Asia',
    flagEmoji: '🇦🇿',
  },
  {
    id: 'SVN',
    iso2: 'SI',
    nameKo: '슬로베니아',
    nameEn: 'Slovenia',
    currencyCode: 'EUR',
    currencyNameEn: 'Euro',
    currencyNameKo: '유로',
    currencySymbol: '€',
    region: 'Europe',
    flagEmoji: '🇸🇮',
  },
  {
    id: 'TKM',
    iso2: 'TM',
    nameKo: '투르크메니스탄',
    nameEn: 'Turkmenistan',
    currencyCode: 'TMT',
    currencyNameEn: 'Turkmenistan Manat',
    currencyNameKo: '투르크메니스탄 마나트',
    currencySymbol: 'm',
    region: 'Asia',
    flagEmoji: '🇹🇲',
  },
  {
    id: 'JOR',
    iso2: 'JO',
    nameKo: '요르단',
    nameEn: 'Jordan',
    currencyCode: 'JOD',
    currencyNameEn: 'Jordanian Dinar',
    currencyNameKo: '요르단 디나르',
    currencySymbol: 'JD',
    region: 'Asia',
    flagEmoji: '🇯🇴',
  },
  {
    id: 'UGA',
    iso2: 'UG',
    nameKo: '우간다',
    nameEn: 'Uganda',
    currencyCode: 'UGX',
    currencyNameEn: 'Ugandan Shilling',
    currencyNameKo: '우간다 실링',
    currencySymbol: 'USh',
    region: 'Africa',
    flagEmoji: '🇺🇬',
  },
  {
    id: 'BOL',
    iso2: 'BO',
    nameKo: '볼리비아',
    nameEn: 'Bolivia',
    currencyCode: 'BOB',
    currencyNameEn: 'Bolivian Boliviano',
    currencyNameKo: '볼리비아 볼리비아노',
    currencySymbol: 'Bs.',
    region: 'Americas',
    flagEmoji: '🇧🇴',
  },
  {
    id: 'CMR',
    iso2: 'CM',
    nameKo: '카메룬',
    nameEn: 'Cameroon',
    currencyCode: 'XAF',
    currencyNameEn: 'Central African CFA Franc',
    currencyNameKo: '중앙아프리카 CFA 프랑',
    currencySymbol: 'FCFA',
    region: 'Africa',
    flagEmoji: '🇨🇲',
  },
  {
    id: 'TUN',
    iso2: 'TN',
    nameKo: '튀니지',
    nameEn: 'Tunisia',
    currencyCode: 'TND',
    currencyNameEn: 'Tunisian Dinar',
    currencyNameKo: '튀니지 디나르',
    currencySymbol: 'DT',
    region: 'Africa',
    flagEmoji: '🇹🇳',
  },
  {
    id: 'MAC',
    iso2: 'MO',
    nameKo: '마카오',
    nameEn: 'Macao SAR',
    currencyCode: 'MOP',
    currencyNameEn: 'Macanese Pataca',
    currencyNameKo: '마카오 파타카',
    currencySymbol: 'MOP$',
    region: 'Asia',
    flagEmoji: '🇲🇴',
  },
  {
    id: 'BHR',
    iso2: 'BH',
    nameKo: '바레인',
    nameEn: 'Bahrain',
    currencyCode: 'BHD',
    currencyNameEn: 'Bahraini Dinar',
    currencyNameKo: '바레인 디나르',
    currencySymbol: 'BD',
    region: 'Asia',
    flagEmoji: '🇧🇭',
  },
  {
    id: 'LVA',
    iso2: 'LV',
    nameKo: '라트비아',
    nameEn: 'Latvia',
    currencyCode: 'EUR',
    currencyNameEn: 'Euro',
    currencyNameKo: '유로',
    currencySymbol: '€',
    region: 'Europe',
    flagEmoji: '🇱🇻',
  },
  {
    id: 'EST',
    iso2: 'EE',
    nameKo: '에스토니아',
    nameEn: 'Estonia',
    currencyCode: 'EUR',
    currencyNameEn: 'Euro',
    currencyNameKo: '유로',
    currencySymbol: '€',
    region: 'Europe',
    flagEmoji: '🇪🇪',
  },
  {
    id: 'ISL',
    iso2: 'IS',
    nameKo: '아이슬란드',
    nameEn: 'Iceland',
    currencyCode: 'ISK',
    currencyNameEn: 'Icelandic Króna',
    currencyNameKo: '아이슬란드 크로나',
    currencySymbol: 'kr',
    region: 'Europe',
    flagEmoji: '🇮🇸',
  },
  {
    id: 'CYP',
    iso2: 'CY',
    nameKo: '키프로스',
    nameEn: 'Cyprus',
    currencyCode: 'EUR',
    currencyNameEn: 'Euro',
    currencyNameKo: '유로',
    currencySymbol: '€',
    region: 'Europe',
    flagEmoji: '🇨🇾',
  },
  {
    id: 'OMN',
    iso2: 'OM',
    nameKo: '오만',
    nameEn: 'Oman',
    nameJa: 'オマーン',
    nameEs: 'Omán',
    nameZh: '阿曼',
    currencyCode: 'OMR',
    currencyNameEn: 'Omani Rial',
    currencyNameKo: '오만 리알',
    currencyNameJa: 'オマーン・リアル',
    currencyNameEs: 'Rial omaní',
    currencyNameZh: '阿曼里亚尔',
    currencySymbol: 'ر.ع.',
    region: 'Asia',
    flagEmoji: '🇴🇲',
  },
  {
    id: 'PRY',
    iso2: 'PY',
    nameKo: '파라과이',
    nameEn: 'Paraguay',
    nameJa: 'パラグアイ',
    nameEs: 'Paraguay',
    nameZh: '巴拉圭',
    currencyCode: 'PYG',
    currencyNameEn: 'Paraguayan Guaraní',
    currencyNameKo: '파라과이 과라니',
    currencyNameJa: 'パラグアイ・グアラニー',
    currencyNameEs: 'Guaraní paraguayo',
    currencyNameZh: '巴拉圭瓜拉尼',
    currencySymbol: '₲',
    region: 'Americas',
    flagEmoji: '🇵🇾',
  },
  {
    id: 'KHM',
    iso2: 'KH',
    nameKo: '캄보디아',
    nameEn: 'Cambodia',
    nameJa: 'カンボジア',
    nameEs: 'Camboya',
    nameZh: '柬埔寨',
    currencyCode: 'KHR',
    currencyNameEn: 'Cambodian Riel',
    currencyNameKo: '캄보디아 리엘',
    currencyNameJa: 'カンボジア・リエル',
    currencyNameEs: 'Riel camboyano',
    currencyNameZh: '柬埔寨瑞尔',
    currencySymbol: '៛',
    region: 'Asia',
    flagEmoji: '🇰🇭',
  },
  {
    id: 'NPL',
    iso2: 'NP',
    nameKo: '네팔',
    nameEn: 'Nepal',
    nameJa: 'ネパール',
    nameEs: 'Nepal',
    nameZh: '尼泊尔',
    currencyCode: 'NPR',
    currencyNameEn: 'Nepalese Rupee',
    currencyNameKo: '네팔 루피',
    currencyNameJa: 'ネパール・ルピー',
    currencyNameEs: 'Rupia nepalí',
    currencyNameZh: '尼泊尔卢比',
    currencySymbol: '₨',
    region: 'Asia',
    flagEmoji: '🇳🇵',
  },
  {
    id: 'SEN',
    iso2: 'SN',
    nameKo: '세네갈',
    nameEn: 'Senegal',
    nameJa: 'セネガル',
    nameEs: 'Senegal',
    nameZh: '塞内加尔',
    currencyCode: 'XOF',
    currencyNameEn: 'West African CFA Franc',
    currencyNameKo: '서아프리카 CFA 프랑',
    currencyNameJa: '西アフリカCFAフラン',
    currencyNameEs: 'Franco CFA de África Occidental',
    currencyNameZh: '西非法郎',
    currencySymbol: 'CFA',
    region: 'Africa',
    flagEmoji: '🇸🇳',
  },
  {
    id: 'GUY',
    iso2: 'GY',
    nameKo: '가이아나',
    nameEn: 'Guyana',
    nameJa: 'ガイアナ',
    nameEs: 'Guyana',
    nameZh: '圭亚那',
    currencyCode: 'GYD',
    currencyNameEn: 'Guyanese Dollar',
    currencyNameKo: '가이아나 달러',
    currencyNameJa: 'ガイアナ・ドル',
    currencyNameEs: 'Dólar guyanés',
    currencyNameZh: '圭亚那元',
    currencySymbol: '$',
    region: 'Americas',
    flagEmoji: '🇬🇾',
  },
  {
    id: 'PNG',
    iso2: 'PG',
    nameKo: '파푸아뉴기니',
    nameEn: 'Papua New Guinea',
    nameJa: 'パプアニューギニア',
    nameEs: 'Papúa Nueva Guinea',
    nameZh: '巴布亚新几内亚',
    currencyCode: 'PGK',
    currencyNameEn: 'Papua New Guinean Kina',
    currencyNameKo: '파푸아뉴기니 키나',
    currencyNameJa: 'パプアニューギニア・キナ',
    currencyNameEs: 'Kina de Papúa Nueva Guinea',
    currencyNameZh: '巴布亚新几内亚基那',
    currencySymbol: 'K',
    region: 'Oceania',
    flagEmoji: '🇵🇬',
  },
  {
    id: 'GEO',
    iso2: 'GE',
    nameKo: '조지아',
    nameEn: 'Georgia',
    nameJa: 'ジョージア',
    nameEs: 'Georgia',
    nameZh: '格鲁吉亚',
    currencyCode: 'GEL',
    currencyNameEn: 'Georgian Lari',
    currencyNameKo: '조지아 라리',
    currencyNameJa: 'ジョージア・ラリ',
    currencyNameEs: 'Lari georgiano',
    currencyNameZh: '格鲁吉亚拉里',
    currencySymbol: '₾',
    region: 'Europe',
    flagEmoji: '🇬🇪',
  },
  {
    id: 'MNG',
    iso2: 'MN',
    nameKo: '몽골',
    nameEn: 'Mongolia',
    nameJa: 'モンゴル',
    nameEs: 'Mongolia',
    nameZh: '蒙古',
    currencyCode: 'MNT',
    currencyNameEn: 'Mongolian Tögrög',
    currencyNameKo: '몽골 투그릭',
    currencyNameJa: 'モンゴル・トゥグルグ',
    currencyNameEs: 'Tugrik mongol',
    currencyNameZh: '蒙古图格里克',
    currencySymbol: '₮',
    region: 'Asia',
    flagEmoji: '🇲🇳',
  },
  {
    id: 'SLV',
    iso2: 'SV',
    nameKo: '엘살바도르',
    nameEn: 'El Salvador',
    nameJa: 'エルサルバドル',
    nameEs: 'El Salvador',
    nameZh: '萨尔瓦多',
    currencyCode: 'USD',
    currencyNameEn: 'US Dollar',
    currencyNameKo: '미국 달러',
    currencyNameJa: '米ドル',
    currencyNameEs: 'Dólar estadounidense',
    currencyNameZh: '美元',
    currencySymbol: '$',
    region: 'Americas',
    flagEmoji: '🇸🇻',
  },
  {
    id: 'LBY',
    iso2: 'LY',
    nameKo: '리비아',
    nameEn: 'Libya',
    nameJa: 'リビア',
    nameEs: 'Libia',
    nameZh: '利比亚',
    currencyCode: 'LYD',
    currencyNameEn: 'Libyan Dinar',
    currencyNameKo: '리비아 디나르',
    currencyNameJa: 'リビア・ディナール',
    currencyNameEs: 'Dinar libio',
    currencyNameZh: '利比亚第纳尔',
    currencySymbol: 'ل.د',
    region: 'Africa',
    flagEmoji: '🇱🇾',
  },
]

export const BASE_CURRENCIES = [
  { code: 'USD', nameEn: 'US Dollar (USD)', nameKo: '미국 달러 (USD)', nameJa: '米ドル (USD)', nameEs: 'Dólar estadounidense (USD)', nameZh: '美元 (USD)', symbol: '$' },
  { code: 'EUR', nameEn: 'Euro (EUR)', nameKo: '유로 (EUR)', nameJa: 'ユーロ (EUR)', nameEs: 'Euro (EUR)', nameZh: '欧元 (EUR)', symbol: '€' },
  { code: 'KRW', nameEn: 'Korean Won (KRW)', nameKo: '원화 (KRW)', nameJa: '韓国ウォン (KRW)', nameEs: 'Won surcoreano (KRW)', nameZh: '韩元 (KRW)', symbol: '₩' },
  { code: 'JPY', nameEn: 'Japanese Yen (JPY)', nameKo: '일본 엔 (JPY)', nameJa: '日本円 (JPY)', nameEs: 'Yen japonés (JPY)', nameZh: '日元 (JPY)', symbol: '¥' },
  { code: 'GBP', nameEn: 'British Pound (GBP)', nameKo: '영국 파운드 (GBP)', nameJa: '英ポンド (GBP)', nameEs: 'Libra esterlina (GBP)', nameZh: '英镑 (GBP)', symbol: '£' },
  { code: 'CNY', nameEn: 'Chinese Yuan (CNY)', nameKo: '중국 위안 (CNY)', nameJa: '中国人民元 (CNY)', nameEs: 'Yuan chino (CNY)', nameZh: '人民币 (CNY)', symbol: '¥' },
  { code: 'CHF', nameEn: 'Swiss Franc (CHF)', nameKo: '스위스 프랑 (CHF)', nameJa: 'スイス・フラン (CHF)', nameEs: 'Franco suizo (CHF)', nameZh: '瑞士法郎 (CHF)', symbol: 'CHF' },
  { code: 'AUD', nameEn: 'Australian Dollar (AUD)', nameKo: '호주 달러 (AUD)', nameJa: '豪ドル (AUD)', nameEs: 'Dólar australiano (AUD)', nameZh: '澳元 (AUD)', symbol: 'A$' },
] as const

